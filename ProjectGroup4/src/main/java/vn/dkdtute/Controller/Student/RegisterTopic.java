	...
	private void RegTopic(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for (Topic tp : topicList) {
			if (tp.getLeaderid() != null || tp.getMemberid() != null) {
				System.out.println(tp.getTopicid());
//				System.out.println(student.getUserid()+"-"+tp.getLeaderid()+"-"+tp.getMemberid());
				if (student.getUserid().equals(tp.getLeaderid()) || student.getUserid().equals(tp.getMemberid())) {
		...
		System.out.println(student.getUserid() + topicId);
