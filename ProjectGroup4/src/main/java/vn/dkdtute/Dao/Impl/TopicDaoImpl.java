...
public class TopicDaoImpl extends ConnectJDBC implements ITopicDao {
	...
	// Đếm số đề tài theo niên khoá và chuyên ngành của từng sinh viên
	@Override
	public int countByStudentInfo(Users student) {
		String sql = "Select count(topicname) as 'Tổng số đề tài được duyệt' From Topic Where major = ? AND schoolyear = 2000+? AND topicstatus = 1";
		try {
			Connection conn = super.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
		...
		try {
			...
			if (topic.getNumofstudent() < 2) {
				if (topic.getNumofstudent() == 0) {
					sql ="UPDATE Topic set numofstudent = ?, leaderid = ? Where topicid = ?";
					ps = conn.prepareStatement(sql);
					ps.setInt(1, 1);
				} else {
					sql ="UPDATE Topic set numofstudent = ?, memberid = ? Where topicid = ?";
					ps = conn.prepareStatement(sql);
					ps.setInt(1, 2);
				}
	...
	public int countByLectCreate(Users lect) {
		String sql = "Select count(topicname) as 'totalTopic' From Topic Where instructorid = ? AND topicstatus = 0";
		try {
			Connection conn = super.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, lect.getUserid());
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				return rs.getInt("totalTopic");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<Topic> findByLectCreate(Users lect, int index, int pagesize) {
		List<Topic> topics = new ArrayList<Topic>();
		String sql = "SELECT * FROM Topic Where instructorid = ? AND topicstatus = 0 "
				+ "ORDER BY topicname asc OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
		try {
			Connection conn = super.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
